...
--
-- Host: localhost:3306
-- Generation Time: Nov 02, 2022 at 02:18 PM
...
(1, 'ABCD123', 'abc@gmail.com', '$2y$10$gIXOFLrfmwWMRDvys5EOY.B4lzJagCEZlxQiXeRRWvt3S5.U92QZO', 1, 'DHARMAPUR SKUS LTD abcd', 'DHARMAPUR, P.O- DHARMAPUR  Pin no: 700107', '19AABAD6387P3ZW', '772292', NULL, '2', '1', 'abc@gmail.com', '2022-11-02 07:08:07', 'abc@gmail.com', '2022-11-02 01:42:30');
...
--
-- Stand-in structure for view `v_company_dtls`
-- (See below for the actual view)
--
CREATE TABLE `v_company_dtls` (
`COMP_ID` int(11)
,`COMP_NAME` varchar(100)
,`short_name` varchar(100)
,`COMP_PN_NO` varchar(30)
,`COMP_EMAIL_ID` varchar(50)
,`CREATED_BY` varchar(50)
,`CREATED_DT` datetime
...
--
-- Stand-in structure for view `v_soc_opening`
-- (See below for the actual view)
--
CREATE TABLE `v_soc_opening` (
`sl_no` int(11)
,`br_cd` varchar(5)
,`op_dt` date
,`soc_id` varchar(10)
...
--
-- Structure for view `v_company_dtls`
--
DROP TABLE IF EXISTS `v_company_dtls`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_company_dtls`  AS  select `benfed_fertilizer`.`mm_company_dtls`.`COMP_ID` AS `COMP_ID`,`benfed_fertilizer`.`mm_company_dtls`.`COMP_NAME` AS `COMP_NAME`,`benfed_fertilizer`.`mm_company_dtls`.`short_name` AS `short_name`,`benfed_fertilizer`.`mm_company_dtls`.`COMP_PN_NO` AS `COMP_PN_NO`,`benfed_fertilizer`.`mm_company_dtls`.`COMP_EMAIL_ID` AS `COMP_EMAIL_ID`,`benfed_fertilizer`.`mm_company_dtls`.`CREATED_BY` AS `CREATED_BY`,`benfed_fertilizer`.`mm_company_dtls`.`CREATED_DT` AS `CREATED_DT`,`benfed_fertilizer`.`mm_company_dtls`.`MODIFIED_BY` AS `MODIFIED_BY`,`benfed_fertilizer`.`mm_company_dtls`.`MODIFIED_DT` AS `MODIFIED_DT`,`benfed_fertilizer`.`mm_company_dtls`.`COMP_ADD` AS `COMP_ADD`,`benfed_fertilizer`.`mm_company_dtls`.`PAN_NO` AS `PAN_NO`,`benfed_fertilizer`.`mm_company_dtls`.`GST_NO` AS `GST_NO`,`benfed_fertilizer`.`mm_company_dtls`.`CIN` AS `CIN`,`benfed_fertilizer`.`mm_company_dtls`.`MFMS` AS `MFMS`,`benfed_fertilizer`.`mm_company_dtls`.`acc_cd` AS `acc_cd`,`benfed_fertilizer`.`mm_company_dtls`.`adv_acc` AS `adv_acc`,`benfed_fertilizer`.`mm_company_dtls`.`tds_acc` AS `tds_acc` from `benfed_fertilizer`.`mm_company_dtls` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_sale`  AS  select `benfed_fertilizer`.`td_sale`.`trans_do` AS `trans_do`,`benfed_fertilizer`.`td_sale`.`trans_no` AS `trans_no`,`benfed_fertilizer`.`td_sale`.`do_dt` AS `do_dt`,`benfed_fertilizer`.`td_sale`.`no_of_days` AS `no_of_days`,`benfed_fertilizer`.`td_sale`.`sale_due_dt` AS `sale_due_dt`,`benfed_fertilizer`.`td_sale`.`trans_type` AS `trans_type`,`benfed_fertilizer`.`td_sale`.`soc_id` AS `soc_id`,`benfed_fertilizer`.`td_sale`.`comp_id` AS `comp_id`,`benfed_fertilizer`.`td_sale`.`sale_ro` AS `sale_ro`,`benfed_fertilizer`.`td_sale`.`prod_id` AS `prod_id`,`benfed_fertilizer`.`td_sale`.`unit` AS `unit`,`benfed_fertilizer`.`td_sale`.`catg_id` AS `catg_id`,`benfed_fertilizer`.`td_sale`.`stock_point` AS `stock_point`,`benfed_fertilizer`.`td_sale`.`gov_sale_rt` AS `gov_sale_rt`,`benfed_fertilizer`.`td_sale`.`qty` AS `qty`,`benfed_fertilizer`.`td_sale`.`sale_rt` AS `sale_rt`,`benfed_fertilizer`.`td_sale`.`base_price` AS `base_price`,`benfed_fertilizer`.`td_sale`.`taxable_amt` AS `taxable_amt`,`benfed_fertilizer`.`td_sale`.`cgst` AS `cgst`,`benfed_fertilizer`.`td_sale`.`sgst` AS `sgst`,`benfed_fertilizer`.`td_sale`.`dis` AS `dis`,`benfed_fertilizer`.`td_sale`.`tot_amt` AS `tot_amt`,`benfed_fertilizer`.`td_sale`.`round_tot_amt` AS `round_tot_amt`,`benfed_fertilizer`.`td_sale`.`paid_amt` AS `paid_amt`,`benfed_fertilizer`.`td_sale`.`irn` AS `irn`,`benfed_fertilizer`.`td_sale`.`irn_cnl_reason` AS `irn_cnl_reason`,`benfed_fertilizer`.`td_sale`.`irn_cnl_rem` AS `irn_cnl_rem`,`benfed_fertilizer`.`td_sale`.`ack` AS `ack`,`benfed_fertilizer`.`td_sale`.`ack_dt` AS `ack_dt`,`benfed_fertilizer`.`td_sale`.`gst_type_flag` AS `gst_type_flag`,`benfed_fertilizer`.`td_sale`.`created_by` AS `created_by`,`benfed_fertilizer`.`td_sale`.`created_dt` AS `created_dt`,`benfed_fertilizer`.`td_sale`.`modified_by` AS `modified_by`,`benfed_fertilizer`.`td_sale`.`modified_dt` AS `modified_dt`,`benfed_fertilizer`.`td_sale`.`br_cd` AS `br_cd`,`benfed_fertilizer`.`td_sale`.`fin_yr` AS `fin_yr` from `benfed_fertilizer`.`td_sale` ;
...
--
-- Structure for view `v_soc_opening`
--
DROP TABLE IF EXISTS `v_soc_opening`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_soc_opening`  AS  select `benfed_fertilizer`.`td_soc_opening`.`sl_no` AS `sl_no`,`benfed_fertilizer`.`td_soc_opening`.`br_cd` AS `br_cd`,`benfed_fertilizer`.`td_soc_opening`.`op_dt` AS `op_dt`,`benfed_fertilizer`.`td_soc_opening`.`soc_id` AS `soc_id`,`benfed_fertilizer`.`td_soc_opening`.`soc_name` AS `soc_name`,`benfed_fertilizer`.`td_soc_opening`.`balance` AS `balance` from `benfed_fertilizer`.`td_soc_opening` ;
